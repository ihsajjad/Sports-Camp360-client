...
const PopularClasses = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (user?.email) {
        const matchedUser = await axiosSecure.get(
          `/users/user?email=${user?.email}`
        );
        if (
          matchedUser?.data?.role === "instructor" ||
          matchedUser?.data?.role === "admin"
        ) {
  ...
  return (
    <div className="md:py-12 py-8  md:px-20 px-2 bg-[#EBF5F6]">
      <SectionTitle
        ...
        subTitle={"Discover the Best Classes at Sports Camp 360"}
      >
        {" "}
      </SectionTitle>
      ...
      <div className="text-center">
        <Link to="/classes" className="custom-btn-outline">
          See All Classes
        </Link>
      </div>
    </div>
