...
const SingleClass = ({ singleClass, handleSelect }) => {
    ...
    return (
        <div className={` border border-[#fb00d993] rounded-lg ${availableSeats === 0 && 'bg-red-300'}`}>
            ...
            <div className="p-4 space-y-2">
                ...
                <button onClick={() => handleSelect(singleClass)} className="custom-btn-outline" disabled={availableSeats === 0}>{` ${availableSeats === 0 ? 'Unavailable' : 'Select'}`}</button>
            </div>
        </div>
    );
};
